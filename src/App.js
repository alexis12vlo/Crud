...
eliminar=(dato)=>{
  var opcion=window.confirm("Realmente Desea Eliminar el Personaje"+dato.id);
  var contador=0;
  var lista=this.state.data;
  lista.map((registro)=>{
    if(dato.id==registro.id){
  ...
  render(){
    ...
    return(
      ...
      <Container>
      <br/>
      <Button onClick={()=>this.mostrarModalInsertar()} color="success">Insertar Nuevo Personaje</Button>
      <br/><br/>
        ...
        <tbody>
          ...
          {this.state.data.map((elemento)=>(
            <tr>
              ...
              <td><Button onClick={()=>this.mostrarModalEditar(elemento)} color="primary">Editar</Button>{"   "}<Button onClick={()=>this.eliminar(elemento)} color="danger">Eliminar</Button></td>
